...
You cannot get a deterministic security guarantee out of a probabilistic component. A model that produces a different output for the same input cannot be the thing that reliably enforces a binary policy. This is not a defect to be patched. It is the nature of learned statistical inference, and every design decision in this repo is a response to it.
...
These fail when the adversary shapes input to evade the classifier. The critical property is that they usually fail *silently*: evasion produces no event, so you do not learn the control was beaten.
